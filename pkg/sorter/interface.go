...
// HasSort is an interface that defines methods required for models that support sorting.
// Models implementing this interface can be used with sorting logic to adjust their position
...
type HasSort interface {
	...
	//   1. A pointer to an integer representing the new sort value for the model.
	//      If the pointer is nil, no sorting adjustment is needed.
	//   2. A map of field names to their values, used to filter records in the database.
	//      These fields are typically used in the WHERE clause of SQL queries to ensure
