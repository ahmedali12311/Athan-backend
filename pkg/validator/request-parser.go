...
func (v *Validator) Parse(req *http.Request) error {
	defaultMax, err := bytes.Parse(config.MaxFormMemory)
	if err != nil {
		return err
	}
	data, err := v.ParseMax(req, defaultMax)
	if err != nil {
		return err
	}
	v.Data = data
...
func (v *Validator) ParseMax(req *http.Request, max int64) (*Data, error) {
	data := newData()
...
func (d Data) GetAndUnmarshalJSON(key string, v any) error {
	return json.Unmarshal([]byte(d.Get(key)), v)
}
