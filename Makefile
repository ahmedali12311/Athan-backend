...
migrate/up:
	migrate -path=./database/migrations -database $(CONNECTION_STRING) up $(n)

## migrate/up/all: migrates up to latest
migrate/up/all:
	migrate -path=./database/migrations -database $(CONNECTION_STRING) up

## migrate/down n=<number>: migrates down n steps
migrate/down:
	migrate -path=./database/migrations -database $(CONNECTION_STRING) down $(n)

## migrate/down/all: migrates down all steps
migrate/down/all:
	migrate -path=./database/migrations -database $(CONNECTION_STRING) down -all

## migration n=<file_name>: creates migration files up/down for file_name
migration:
	migrate create -seq -ext=.sql -dir=./database/migrations $(n)
